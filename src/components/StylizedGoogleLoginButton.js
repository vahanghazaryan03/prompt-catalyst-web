...
const StylizedGoogleLoginButton = ({ onLoginStart, onLoginComplete }) => {
  ...
  // Simple button that matches the dark theme
    ...
    return (
      <div className="w-full">
        <button
          ...
          disabled={isLoading}
          type="button"
          className="w-full py-2.5 px-4 bg-[var(--cardBackground)] border border-[var(--border)] text-[var(--text)] rounded-lg font-medium flex items-center justify-center gap-3 relative"
        >
          {isLoading ? (
...
export default StylizedGoogleLoginButton;